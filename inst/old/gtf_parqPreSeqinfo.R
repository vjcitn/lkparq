...
#' @rawNamespace import(GenomicRanges, except=c(union, intersect, setdiff))
#' @import arrow
#' @import dplyr
...
#' 
#' Uses rle + relist for O(n) construction instead of split's O(n*k)
#' @keywords internal
.df_to_grangeslist <- function(df, group_col, id_col, mcol_cols = NULL) {
  ...
  # Use rle + splitAsList for efficient grouping
  # This is much faster than split() because it uses run-length encoding
  # Data must already be sorted by group_col!
  
  # Calculate group boundaries using rle
  rle_groups <- rle(groups)
  group_names <- rle_groups$values
  group_lengths <- rle_groups$lengths
  
  # Create partitioning
  part <- IRanges::PartitioningByEnd(cumsum(group_lengths), names = group_names)
  
  # Relist into GRangesList (very fast - no copying)
  grl <- IRanges::relist(gr, part)
